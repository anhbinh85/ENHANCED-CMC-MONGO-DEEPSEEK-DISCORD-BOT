 ...
 * @param {object} params - Query parameters (excluding the token).
 * @returns {Promise<object>} - The data from the API response.
 * @throws {Error} If the API key is missing or the request fails. Includes '[BCR]' prefix for user-facing errors.
 */
async function makeRequest(endpoint, params = {}) {
    if (!API_KEY) {
         throw new Error("[BCR] Blockcypher API Key (BLOCKCYPHER_API_KEY) is not configured.");
    }
    ...
    try {
        ...
        console.error(`[BlockcypherHelper] Error requesting ${url}:`, error.message);
        let specificMessage = 'Unknown API error';
        let statusCode = 'Network Error';

        if (axios.isAxiosError(error)) {
            statusCode = error.response?.status || 'Network Error';
            const errorData = error.response?.data;
            specificMessage = errorData?.error || JSON.stringify(errorData) || error.message;

            if (statusCode === 401) specificMessage = "Invalid Blockcypher API Key.";
            else if (statusCode === 403) specificMessage = "API Key forbidden (check permissions/plan).";
            else if (statusCode === 404) specificMessage = "Resource not found (check address/tx hash/endpoint).";
            else if (statusCode === 429) specificMessage = "Rate Limit hit. Please wait.";
            else if (statusCode >= 500) specificMessage = `Blockcypher server error (${statusCode}). Please try again later.`;
        } else {
            // Handle non-axios errors
            specificMessage = `Request setup error: ${error.message}`;
        }
        // Prepend prefix to the error message thrown
        throw new Error(`[BCR] API Error (${statusCode}): ${specificMessage}`);
    }
}
...
/**
 * Gets general information about the blockchain, including fee estimates.
 * @returns {Promise<object>} - Blockchain info object (contains name, height, hash, time, fee info etc.).
 */
async function getBlockchainFeeInfo() {
    console.log(`[BlockcypherHelper] Fetching blockchain info for ${COIN}/${CHAIN} (includes fees)...`);
    // The base endpoint provides chain info including fee estimates
 ...
 */
async function getAddressDetails(address) {
    if (!address) throw new Error("[BCR] Address is required for getAddressDetails.");
    console.log(`[BlockcypherHelper] Fetching full details for address: ${address}`);
    return await makeRequest(`/addrs/${address}`);
}
 ...
 */
async function getAddressBalance(address) {
     if (!address) throw new Error("[BCR] Address is required for getAddressBalance.");
     console.log(`[BlockcypherHelper] Fetching balance for address: ${address}`);
     try {
         const details = await makeRequest(`/addrs/${address}`);
         return {
          ...
          console.error(`[BlockcypherHelper] Failed to get balance details for ${address}:`, error);
          // Error from makeRequest already has [BCR] prefix
          throw error; // Re-throw caught error
     }
}
 ...
 */
async function getTransactionDetails(txHash) {
    if (!txHash) throw new Error("[BCR] Transaction hash is required for getTransactionDetails.");
    console.log(`[BlockcypherHelper] Fetching details for transaction: ${txHash}`);
    return await makeRequest(`/txs/${txHash}`);
}
...
module.exports = {
    ...
    getAddressDetails,
    getTransactionDetails,
    getBlockchainFeeInfo // Export the new function
    // getBlockchainInfo was renamed to getBlockchainFeeInfo for clarity
};